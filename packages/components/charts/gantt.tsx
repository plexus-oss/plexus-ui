...
const TaskRow = React.memo(({ taskGroup, index, leftPanelWidth, variant }: TaskRowProps) => {
            ...
            {isHovered && (
              <g>
                ...
                <text x={x1 + 4} y={y - 12} fontSize={10} fill="#ffffff" opacity={0.8}>
                  ...
                  {formatTimeInZone(end, timezone, use12HourFormat)} ({durationMinutes}m)
                </text>
                {/* Task-specific description in tooltip */}
                {task.description && (
                  <text x={x1 + 4} y={y} fontSize={9} fill="#ffffff" opacity={0.7}>
                    {task.description}
                  </text>
                )}
              </g>
