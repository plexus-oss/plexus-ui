 ...
 */
export function buildOctree(data, options = {}) {
    ...
    const spacing = initialSpacing || Math.sqrt((halfSize * halfSize * 2) / (data.positions.length / 3));
    // Create root node
    const root = {
        boundingBox,
        center,
 ...
 */
function getOctantIndex(point, center) {
    let index = 0;
    if (point.x >= center.x)
        index |= 1;
    if (point.y >= center.y)
 ...
 */
function getOctantCenter(parentCenter, parentSize, octantIndex) {
 ...
 */
function insertPoint(node, pointIndex, data, maxPointsPerNode, maxDepth, minNodeSize) {
    const point = new THREE.Vector3(data.positions[pointIndex * 3], data.positions[pointIndex * 3 + 1], data.positions[pointIndex * 3 + 2]);
    ...
    if (node.isLeaf) {
        ...
        if (!child) {
            ...
            const childMin = new THREE.Vector3(childCenter.x - childSize, childCenter.y - childSize, childCenter.z - childSize);
            const childMax = new THREE.Vector3(childCenter.x + childSize, childCenter.y + childSize, childCenter.z + childSize);
            child = {
 ...
 */
function addPointToNode(node, pointIndex, data) {
    ...
    if (data.intensities) {
        ...
        if (node.intensities)
            newIntensities.set(node.intensities);
    ...
    if (data.classifications) {
        ...
        if (node.classifications)
            newClassifications.set(node.classifications);
 ...
 */
function subdivideNode(node, _data, maxPointsPerNode, maxDepth, minNodeSize) {
    node.isLeaf = false;
 ...
 */
export function selectNodesLOD(root, options) {
 ...
 */
export function getTotalPoints(node) {
    if (node.isLeaf) {
 ...
 */
export function mergeNodeData(nodes) {
    let totalPoints = 0;
    ...
    if (hasColors)
        colors = new Uint8Array(totalPoints * 3);
    if (hasIntensities)
        intensities = new Float32Array(totalPoints);
    if (hasClassifications)
        classifications = new Uint8Array(totalPoints);
    let offset = 0;
    for (const node of nodes) {
...
//# sourceMappingURL=point-cloud-octree.js.map