 ...
 * ```
 */
export async function loadXYZ(fileOrUrl) {
 ...
 * ```
 */
export async function loadPCD(fileOrUrl) {
    const text = typeof fileOrUrl === "string"
        ? await fetch(fileOrUrl).then((r) => r.text())
        : await fileOrUrl.text();
...
function parseLASHeader(buffer) {
    ...
    const fileSignature = String.fromCharCode(view.getUint8(0), view.getUint8(1), view.getUint8(2), view.getUint8(3));
 ...
 */
export async function loadLAS(fileOrUrl, options = {}) {
 ...
 */
export function detectFormat(fileOrUrl) {
 ...
 */
export async function loadPointCloud(fileOrUrl, options = {}) {
    ...
    switch (format) {
        ...
        case "laz":
            throw new Error("LAZ (compressed) format requires decompression library. Use LAS format or decompress first.");
        default:
 ...
 */
export function subsamplePointCloud(data, targetPoints) {
...
//# sourceMappingURL=point-cloud-loaders.js.map