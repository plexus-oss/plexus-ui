 ...
 */
export function normalizeData(data, xMin, xMax, yMin, yMax) {
 ...
 */
export function dataToVertexArray(data) {
 ...
 */
export function data3DToVertexArray(data) {
 ...
 */
export function downsampleLTTB(data, targetPoints) {
    ...
    for (let i = 0; i < targetPoints - 2; i++) {
        ...
        let maxArea = -1;
        let maxAreaPoint = data[Math.min(rangeStart, data.length - 1)];
 ...
 */
export function downsampleMinMax(data, targetPoints) {
    if (data.length <= targetPoints)
        return data;
    const result = [];
 ...
 */
export function createOrResizeVertexBuffer(device, data, oldBuffer) {
 ...
 */
export function calculateNiceBounds(data, tickCount = 5) {
 ...
 */
export function createHistogramBins(data, binCount, method = "sturges") {
    if (data.length === 0) {
        return [];
    }
 ...
 */
export function calculateNormalCurve(data, points = 100) {
    if (data.length === 0)
        return [];
    const mean = data.reduce((sum, val) => sum + val, 0) / data.length;
    const stdDev = calculateStdDev(data);
 ...
 */
export function generateSineWave(points, amplitude = 1, frequency = 1, phase = 0) {
    const data = [];
    for (let i = 0; i < points; i++) {
 ...
 */
export function generateTelemetryData(points, baseline = 0, variance = 1, drift = 0) {
    const data = [];
    let value = baseline;
    for (let i = 0; i < points; i++) {
 ...
 */
export function generateCategoricalData(categories, minValue = 0, maxValue = 100) {
 ...
 */
export function getDomain(points, accessor, addPadding = false) {
    if (points.length === 0) {
        return [0, 1];
    }
 ...
 */
export function applyWindowWithCorrection(data, windowType = "hann") {
    const n = data.length;
 ...
 */
export function powerToDb(power, referenceLevel = 1) {
...
`;
export const createGPUFFTCompute = (device, fftSize) => {
    // Validate FFT size
