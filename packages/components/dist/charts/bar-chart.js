...
function createBarGeometry(points, xScale, yScale, color, barWidth, orientation, categoryMap, seriesIndex, totalSeries, grouped, baseValue) {
    const positions = [];
    const colors = [];
    ...
    for (const point of points) {
        ...
        if (orientation === "vertical") {
            ...
            const x = centerX - (grouped ? totalSeries * effectiveBarWidth : effectiveBarWidth) / 2 + barOffset;
            ...
            positions.push(x, yBottom, x + width, yBottom, x, yTop, x + width, yBottom, x + width, yTop, x, yTop);
        }
        else {
            const centerY = yScale(categoryValue);
            const y = centerY - (grouped ? totalSeries * effectiveBarWidth : effectiveBarWidth) / 2 + barOffset;
            ...
            positions.push(xLeft, y, xRight, y, xLeft, y + height, xRight, y, xRight, y + height, xLeft, y + height);
        }
...
function createWebGLBarRenderer(canvas) {
    const buffers = {
        position: null,
        color: null,
    };
    const renderer = createWebGLRenderer({
        ...
        onRender: (gl, program, props) => {
            const { series, xDomain, yDomain, width, height, margin, orientation, barWidth, grouped, categoryMap, } = props;
            ...
            series.forEach((s, seriesIndex) => {
                ...
                const geometry = createBarGeometry(s.data, xScale, yScaleFlipped, color, barWidth, orientation, categoryMap, seriesIndex, series.length, grouped, baseValue);
                if (!buffers.position)
...
function createWebGPUBarRenderer(canvas, device) {
...
// ============================================================================
function Root({ series, xAxis = {}, yAxis = {}, width = 800, height = 400, preferWebGPU = true, orientation = "vertical", barWidth: barWidthProp, barGap = 8, grouped = true, className, children, }) {
    ...
    const barWidth = useMemo(() => {
        ...
        const availableSpace = orientation === "vertical"
    ...
    // Calculate domains
    const numericPoints = series.flatMap((s) => s.data.map((d) => ({ x: categoryMap.get(d.x) ?? 0, y: d.y })));
    const xDomain = orientation === "vertical"
        ...
        : yAxis.domain === "auto" || !yAxis.domain
            ...
            : yAxis.domain;
    const yDomain = orientation === "vertical"
        ? yAxis.domain === "auto" || !yAxis.domain
            ? (() => {
                const [, max] = getDomain(numericPoints, (p) => p.y);
                return [0, max]; // Force min to 0 for bar charts
    ...
    const xAxisWithFormatter = orientation === "vertical" && !xAxis.formatter
        ? {
            ...xAxis,
            formatter: (val) => categoryLabels.get(Math.round(val)) || "",
        }
        : xAxis;
    const yAxisWithFormatter = orientation === "horizontal" && !yAxis.formatter
        ? {
            ...yAxis,
            formatter: (val) => categoryLabels.get(Math.round(val)) || "",
        }
    ...
    return (_jsx(ChartRoot, { width: width, height: height, xAxis: xAxisWithFormatter, yAxis: yAxisWithFormatter, xDomain: xDomain, yDomain: yDomain, xTicks: xTicksCustom, yTicks: yTicksCustom, preferWebGPU: preferWebGPU, className: className, children: _jsx(BarChartContext.Provider, { value: { series, orientation, barWidth, barGap, grouped, categoryMap }, children: children }) }));
}
function Canvas({ showGrid = false }) {
    ...
    React.useEffect(() => {
        ...
        if (!canvas || !renderer || !rendererReady)
            return;
        const dpr = ctx.devicePixelRatio;
        const renderProps = {
            canvas,
...
function Tooltip() {
    ...
    return _jsx(ChartTooltip, { onHover: handleHover });
}
...
// ============================================================================
export function BarChart({ series, xAxis = {}, yAxis = {}, width = 800, height = 400, showGrid = true, showAxes = true, showTooltip = false, preferWebGPU = true, orientation = "vertical", barWidth, // Undefined by default - will use responsive calculation
barGap = 8, grouped = true, className, }) {
    return (_jsxs(Root, { series: series, xAxis: xAxis, yAxis: yAxis, width: width, height: height, preferWebGPU: preferWebGPU, orientation: orientation, barWidth: barWidth, barGap: barGap, grouped: grouped, className: className, children: [_jsx(Canvas, { showGrid: showGrid }), showAxes && _jsx(ChartAxes, {}), showTooltip && _jsx(Tooltip, {})] }));
}
// ============================================================================
