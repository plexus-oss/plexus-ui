...
import { ChartRoot, ChartTooltip, createWebGLRenderer, createWebGPURenderer, hexToRgb, useBaseChart, } from "./base-chart";
...
`;
function createCircleGeometry(centerX, centerY, radius, color, alpha, segments = 64) {
    const positions = [];
    const colors = [];
...
function createSweepGeometry(centerX, centerY, radius, angle, color, sweepWidth = Math.PI / 6) {
    const positions = [];
    const colors = [];
    const segments = 32;
...
function createWebGLRadarRenderer(canvas) {
    const lineBuffers = {
        position: null,
        color: null,
    };
    ...
    const renderer = createWebGLRenderer({
        ...
        onRender: (gl, program, props) => {
            ...
            for (let i = 1; i <= rings; i++) {
                ...
                const { positions, colors } = createCircleGeometry(centerX, centerY, r, gridColor, 0.6 // Increased opacity
                );
            ...
            if (showSweep) {
                ...
                const { positions, colors } = createSweepGeometry(centerX, centerY, radius, sweepAngle, sweepColor);
                linePositions.push(...positions);
                lineColors.push(...colors);
            }
...
async function createWebGPURadarRenderer(canvas, device) {
    const buffers = {
        position: null,
        color: null,
        uniform: null,
...
function Root({ children, series, rings = 4, sectors = 12, showSweep = true, sweepSpeed = 2, showGrid = true, showLabels = true, width = 500, height = 500, preferWebGPU = false, className, }) {
    ...
    return (_jsx(RadarChartContext.Provider, { value: radarData, children: _jsx(ChartRoot, { width: width, height: height, preferWebGPU: preferWebGPU, className: className, margin: { top: 0, right: 0, bottom: 0, left: 0 }, children: children }) }));
}
// ============================================================================
// Canvas Component
// ============================================================================
...
function Labels() {
    ...
    return (_jsx(_Fragment, { children: cardinalDirections.map(({ angle, label }) => {
            const angleRad = (angle * Math.PI) / 180 - Math.PI / 2;
            const x = centerX + radius * Math.cos(angleRad);
            const y = centerY + radius * Math.sin(angleRad);
            return (_jsx("div", { className: "absolute text-sm font-semibold text-zinc-400 pointer-events-none", style: {
                    left: x,
                    top: y,
                    transform: "translate(-50%, -50%)",
                }, children: label }, angle));
        }) }));
}
function Tooltip() {
    const ctx = useRadarChart();
    ...
    return (_jsxs(_Fragment, { children: [_jsx(ChartTooltip, { onHover: handleHover }), ctx.hoveredPoint && (_jsx("div", { className: "absolute pointer-events-none z-20", style: {
                    left: ctx.hoveredPoint.screenX - 8,
                    top: ctx.hoveredPoint.screenY - 8,
                }, children: _jsx("div", { className: "w-4 h-4 rounded-full border-2 border-white dark:border-zinc-900", style: {
...
export function RadarChart(props) {
    return (_jsxs(Root, { ...props, children: [_jsx(Canvas, {}), _jsx(Labels, {}), _jsx(Tooltip, {})] }));
}
