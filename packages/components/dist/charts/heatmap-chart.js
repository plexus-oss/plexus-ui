...
import { ChartAxes, ChartRoot, ChartTooltip, createWebGLRenderer, createWebGPURenderer, hexToRgb, useBaseChart, } from "./base-chart";
const HeatmapChartContext = createContext(null);
...
function createHeatmapGeometry(data, xScale, yScale, xCategoryMap, yCategoryMap, colorScale, minValue, maxValue, cellWidth, cellHeight, cellGap) {
    const positions = [];
    const colors = [];
...
function createWebGLHeatmapRenderer(canvas) {
    const buffers = {
        position: null,
        color: null,
    };
    const renderer = createWebGLRenderer({
        ...
        onRender: (gl, program, props) => {
            const { data, xDomain, yDomain, width, height, margin, xCategoryMap, yCategoryMap, colorScale, minValue, maxValue, cellGap, } = props;
            ...
            const geometry = createHeatmapGeometry(data, xScale, yScaleFlipped, xCategoryMap, yCategoryMap, colorScale, minValue, maxValue, cellWidth, cellHeight, cellGap);
...
function createWebGPUHeatmapRenderer(canvas, device) {
...
// ============================================================================
function Root({ data, xAxis = {}, yAxis = {}, width = 800, height = 400, minWidth, minHeight, maxWidth, maxHeight, aspectRatio, margin, preferWebGPU = true, colorScale = defaultColorScale, minValue, maxValue, cellGap = 2, className, children, }) {
    ...
    const xDomain = [-0.5, xCategories.length - 0.5];
    const yDomain = [-0.5, yCategories.length - 0.5];
    ...
    const yTicksArray = useMemo(() => {
        ...
        if (yCategories.length > 10) {
            ...
            return Array.from({ length: Math.ceil(yCategories.length / step) }, (_, i) => i * step).filter((i) => i < yCategories.length);
        }
        return Array.from({ length: yCategories.length }, (_, i) => i);
    }, [yCategories.length]);
    // Formatters to map indices to category labels
    const xFormatter = useMemo(() => {
        return (xAxis.formatter ||
            ((value) => {
    ...
    const yFormatter = useMemo(() => {
        return (yAxis.formatter ||
            ((value) => {
                const idx = Math.round(value);
                return idx >= 0 && idx < yCategories.length ? String(yCategories[idx]) : "";
            }));
    }, [yCategories, yAxis.formatter]);
    return (_jsx(ChartRoot, { width: width, height: height, minWidth: minWidth, minHeight: minHeight, maxWidth: maxWidth, maxHeight: maxHeight, aspectRatio: aspectRatio, margin: margin, xAxis: { ...xAxis, formatter: xFormatter }, yAxis: { ...yAxis, formatter: yFormatter }, xDomain: xDomain, yDomain: yDomain, xTicks: xTicksArray, yTicks: yTicksArray, preferWebGPU: preferWebGPU, className: className, children: _jsx(HeatmapChartContext.Provider, { value: {
                data,
                xCategories,
                yCategories,
                xCategoryMap,
...
function Tooltip() {
    ...
    const handleHover = (e) => {
        ...
        if (dataPoint) {
            ...
            const cellChanged = !ctx.hoveredPoint ||
                !ctx.tooltipData ||
            ...
            if (cellChanged) {
                ...
                const screenY = ctx.margin.top + (ctx.yCategories.length - yIdx - 1) * cellHeight + cellHeight / 2;
                ctx.setHoveredPoint({
                    seriesIdx: 0,
                    pointIdx: 0,
                    screenX,
...
// ============================================================================
export function HeatmapChart({ data, xAxis = {}, yAxis = {}, width = 800, height = 400, minWidth, minHeight, maxWidth, maxHeight, aspectRatio, margin, showGrid = false, showAxes = true, showTooltip = false, preferWebGPU = true, colorScale = defaultColorScale, minValue, maxValue, cellGap = 2, className, }) {
    return (_jsxs(Root, { data: data, xAxis: xAxis, yAxis: yAxis, width: width, height: height, minWidth: minWidth, minHeight: minHeight, maxWidth: maxWidth, maxHeight: maxHeight, aspectRatio: aspectRatio, margin: margin, preferWebGPU: preferWebGPU, colorScale: colorScale, minValue: minValue, maxValue: maxValue, cellGap: cellGap, className: className, children: [_jsx(Canvas, {}), showGrid && _jsx(Grid, {}), showAxes && _jsx(ChartAxes, {}), showTooltip && _jsx(Tooltip, {})] }));
}
// ============================================================================
