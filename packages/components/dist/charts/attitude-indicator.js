...
const AttitudeIndicatorContext = createContext(null);
...
// Geometry Helpers
// ============================================================================
function createRotatedRectangle(centerX, centerY, width, height, angle, color, alpha = 1) {
    const positions = [];
    const colors = [];
    ...
    // Two triangles
    positions.push(rotatedCorners[0][0], rotatedCorners[0][1], rotatedCorners[1][0], rotatedCorners[1][1], rotatedCorners[2][0], rotatedCorners[2][1]);
    positions.push(rotatedCorners[0][0], rotatedCorners[0][1], rotatedCorners[2][0], rotatedCorners[2][1], rotatedCorners[3][0], rotatedCorners[3][1]);
    for (let i = 0; i < 6; i++) {
        colors.push(...color, alpha);
    }
    return { positions, colors };
}
function createLineGeometry(x1, y1, x2, y2, width, color, alpha = 1) {
    const positions = [];
    const colors = [];
    const dx = x2 - x1;
    const dy = y2 - y1;
    ...
    positions.push(x1 + nx * halfWidth, y1 + ny * halfWidth, x2 + nx * halfWidth, y2 + ny * halfWidth, x1 - nx * halfWidth, y1 - ny * halfWidth);
    positions.push(x2 + nx * halfWidth, y2 + ny * halfWidth, x2 - nx * halfWidth, y2 - ny * halfWidth, x1 - nx * halfWidth, y1 - ny * halfWidth);
    for (let i = 0; i < 6; i++) {
        colors.push(...color, alpha);
    }
    return { positions, colors };
}
function createPitchLadder(centerX, centerY, radius, pitch, roll, pitchStep, color) {
    const positions = [];
    const colors = [];
...
function createBankIndicator(centerX, centerY, radius, roll, color) {
    const positions = [];
    const colors = [];
...
function createAircraftSymbol(centerX, centerY, color) {
    const positions = [];
    const colors = [];
    // Center dot
    const dotRadius = 5;
...
function createWebGLAttitudeRenderer(canvas) {
    const buffers = {
        position: null,
        color: null,
    };
    const renderer = createWebGLRenderer({
        ...
        onRender: (gl, program, props) => {
            const { pitch, roll, showPitchLadder, showBankIndicator, pitchStep, skyColor, groundColor, horizonColor, width, height, } = props;
            ...
            if (skyColor !== "transparent") {
                ...
                const skyGeom = createRotatedRectangle(centerX, centerY - horizonSize / 2 - verticalOffset, horizonSize, horizonSize, rollRad, skyRgb, 1);
                allPositions.push(...skyGeom.positions);
                allColors.push(...skyGeom.colors);
            }
            // Ground (lower half)
            const groundRgb = hexToRgb(groundColor);
            const groundGeom = createRotatedRectangle(centerX, centerY + horizonSize / 2 - verticalOffset, horizonSize, horizonSize, rollRad, groundRgb, 1);
            ...
            if (showPitchLadder) {
                ...
                const ladderGeom = createPitchLadder(centerX, centerY, radius, pitch, roll, pitchStep, ladderColor);
...
async function createWebGPUAttitudeRenderer(canvas, device) {
    const buffers = {
        position: null,
        color: null,
        uniform: null,
...
function Root({ children, pitch = 0, roll = 0, width = 400, height = 400, showPitchLadder = true, showBankIndicator = true, pitchStep = 10, skyColor = "transparent", groundColor = "#19191c", horizonColor = "#9ca3af", preferWebGPU = false, className, }) {
    const attitudeData = {
        pitch,
        roll,
        showPitchLadder,
    ...
    return (_jsx(AttitudeIndicatorContext.Provider, { value: attitudeData, children: _jsx(ChartRoot, { width: width, height: height, preferWebGPU: preferWebGPU, className: className, margin: { top: 0, right: 0, bottom: 0, left: 0 }, children: children }) }));
}
// ============================================================================
// Canvas Component
// ============================================================================
function Canvas() {
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "absolute inset-0", style: { width: "100%", height: "100%" } }));
}
// ============================================================================
...
function ValueDisplay() {
    const ctx = useAttitudeIndicator();
    return (_jsxs("div", { className: "absolute inset-0 pointer-events-none", children: [_jsxs("div", { className: "absolute top-4 left-1/2 -translate-x-1/2 bg-black/60 px-3 py-1 rounded text-sm font-mono", children: [_jsx("span", { className: "text-zinc-400", children: "PITCH:" }), " ", _jsxs("span", { className: "text-white font-bold tabular-nums", children: [ctx.pitch > 0 && "+", ctx.pitch.toFixed(1), "\u00B0"] })] }), _jsxs("div", { className: "absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/60 px-3 py-1 rounded text-sm font-mono", children: [_jsx("span", { className: "text-zinc-400", children: "ROLL:" }), " ", _jsxs("span", { className: "text-white font-bold tabular-nums", children: [ctx.roll > 0 && "+", ctx.roll.toFixed(1), "\u00B0"] })] })] }));
}
// ============================================================================
// Composed Component
// ============================================================================
export function AttitudeIndicator(props) {
    return (_jsx(Root, { ...props, children: _jsx(Canvas, {}) }));
}
...
//# sourceMappingURL=attitude-indicator.js.map