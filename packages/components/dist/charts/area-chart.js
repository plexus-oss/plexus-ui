...
import { ChartAxes, ChartRoot, ChartTooltip, createWebGLRenderer, createWebGPURenderer, getDomain, hexToRgb, useBaseChart, } from "./base-chart";
...
function createAreaGeometry(points, xScale, yScale, color, fillOpacity, baseline, previousY) {
    const positions = [];
    const colors = [];
    if (points.length < 2)
...
function createLineGeometry(points, xScale, yScale, color, lineWidth) {
    const positions = [];
    const colors = [];
    for (let i = 0; i < points.length - 1; i++) {
        const p1 = points[i];
...
function createGridGeometry(xTicks, yTicks, xScale, yScale, width, height) {
    const positions = [];
    const colors = [];
    const isDark = document.documentElement.classList.contains("dark");
    const gridColor = isDark ? [0.4, 0.4, 0.4] : [0.6, 0.6, 0.6];
    // Vertical grid lines
...
function createWebGLAreaRenderer(canvas) {
    const buffers = {
        position: null,
        color: null,
    };
    const renderer = createWebGLRenderer({
        ...
        onRender: (gl, program, props) => {
            const { series, xDomain, yDomain, width, height, margin, showGrid, xTicks, yTicks, stacked } = props;
            ...
            const xScale = (x) => ((x - xDomain[0]) / (xDomain[1] - xDomain[0])) * innerWidth;
            const yScale = (y) => ((y - yDomain[0]) / (yDomain[1] - yDomain[0])) * innerHeight;
            const yScaleFlipped = (y) => innerHeight - yScale(y);
            // Draw grid if enabled
            if (showGrid) {
                const gridGeometry = createGridGeometry(xTicks, yTicks, xScale, yScaleFlipped, innerWidth, innerHeight);
            ...
            for (const s of series) {
                ...
                const previousY = stacked
                    ...
                    : undefined;
                // Draw filled area
                const areaGeometry = createAreaGeometry(s.data, xScale, yScaleFlipped, color, fillOpacity, baseline, previousY);
...
function createWebGPUAreaRenderer(canvas, device) {
    ...
    const areaBufferPool = []; // For area fills
    const lineBufferPool = []; // For stroke lines
    const renderer = createWebGPURenderer({
        ...
        onRender: async (device, context, pipeline, props) => {
            ...
            for (const s of series) {
                ...
                const fillOpacity = s.fillOpacity ?? 0.3;
                const baseline = Math.max(yDomain[0], s.baseline ?? yDomain[0]);
                const previousY = stacked ? (x) => cumulativeY.get(x) ?? baseline : undefined;
                const areaGeometry = createAreaGeometry(s.data, xScale, yScaleFlipped, color, fillOpacity, baseline, previousY);
                if (areaGeometry.positions.length > 0) {
                    const requiredSizes = {
                    ...
                    device.queue.writeBuffer(bufferSet.position, 0, new Float32Array(areaGeometry.positions));
                    device.queue.writeBuffer(bufferSet.color, 0, new Float32Array(areaGeometry.colors));
            ...
            for (const s of series) {
                ...
                if (lineGeometry.positions.length > 0) {
                    ...
                    // Write data to buffers (reusing existing buffers)
                    device.queue.writeBuffer(bufferSet.position, 0, new Float32Array(lineGeometry.positions));
                    device.queue.writeBuffer(bufferSet.color, 0, new Float32Array(lineGeometry.colors));
                    passEncoder.setVertexBuffer(0, bufferSet.position);
                    passEncoder.setVertexBuffer(1, bufferSet.color);
...
// ============================================================================
function Root({ series, xAxis = {}, yAxis = {}, width = 800, height = 400, preferWebGPU = true, stacked = false, className, children, }) {
    ...
    return (_jsx(ChartRoot, { width: width, height: height, xAxis: xAxis, yAxis: yAxis, xDomain: xDomain, yDomain: yDomain, preferWebGPU: preferWebGPU, className: className, children: _jsx(AreaChartContext.Provider, { value: { series, stacked }, children: children }) }));
}
function Canvas({ showGrid = false }) {
    ...
    useEffect(() => {
        ...
        if (!canvas || !renderer || !ctx.renderMode || !rendererReady || !ctx.isVisible)
            return;
        const dpr = ctx.devicePixelRatio;
        let rafId = null;
        let rendering = false;
    ...
    return (_jsx("canvas", { ref: ctx.canvasRef, className: "absolute inset-0", style: {
            width: `${ctx.width}px`,
...
function Tooltip() {
    ...
    const handleHover = (e) => {
        ...
        if (closestSeriesIdx !== -1 && closestPoint) {
            ...
            const currentX = ctx.hoveredPoint?.data?.xValue;
    ...
    const dot = useMemo(() => {
        const hoveredData = ctx.hoveredPoint?.data;
        if (!hoveredData?.point)
            return null;
        ...
        const currentPoint = series.data.find((p) => Math.abs(p.x - point.x) < 0.0001);
        if (currentPoint) {
            return {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(ChartTooltip, { onHover: handleHover }), ctx.hoveredPoint && ctx.tooltipData && dot && (_jsx("div", { className: "absolute pointer-events-none z-20", style: {
                    left: dot.screenX - 6,
                    top: dot.screenY - 6,
                }, children: _jsx("div", { className: "w-3 h-3 rounded-full border-2 border-white dark:border-zinc-900", style: { backgroundColor: dot.color } }) }))] }));
}
...
// ============================================================================
export function AreaChart({ series, xAxis = {}, yAxis = {}, width = 800, height = 400, showGrid = true, showAxes = true, showTooltip = false, preferWebGPU = true, stacked = false, className, }) {
    return (_jsxs(Root, { series: series, xAxis: xAxis, yAxis: yAxis, width: width, height: height, preferWebGPU: preferWebGPU, stacked: stacked, className: className, children: [_jsx(Canvas, { showGrid: showGrid }), showAxes && _jsx(ChartAxes, {}), showTooltip && _jsx(Tooltip, {})] }));
}
// ============================================================================
